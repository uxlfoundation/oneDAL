...
inline void compute_histogram(const local_accessor_rw_t<hist_type_t>& hist,
    ...
    const Index id = item.get_local_id(1);
...
sycl::event train_splitter_impl<Float, Bin, Index, Task>::best_split(
    ...
    const auto nd_range = bk::make_multiple_nd_range_2d(
        {node_count, local_size},
        {1, local_size}
    );

    const auto best_ftr_hists = pr::ndarray<hist_type_t, 1>::empty(
        queue, {node_count * ftr_count * hist_prop_count}, alloc::device);
    hist_type_t* const hists_ptr = best_ftr_hists.get_mutable_data();
    ...
    sycl::event last_event = queue.submit([&](sycl::handler& cgh) {
        ...
        local_accessor_rw_t<hist_type_t> local_hist(bin_block * hist_prop_count, cgh);
        local_accessor_rw_t<Float>       local_weight(bin_block, cgh);
        local_accessor_rw_t<split_scalar_t> local_bin_scalars(bin_block, cgh);

        local_accessor_rw_t<split_scalar_t> best_node_split(1, cgh);
        local_accessor_rw_t<Index> best_ftr_idx(1, cgh);

        cgh.parallel_for(nd_range, [=](sycl::nd_item<2> item) {
            const Index node_id = item.get_global_id(0);
            const Index local_id = item.get_local_id(1);
            const Index wg_size = item.get_local_range(1);
            ...
            if (!sp_hlp.is_valid_impurity(node_imp_list_ptr, node_id, imp_threshold, row_count) ||
                row_count < 2 * min_obs_leaf) {
                return;
            }

            if (local_id == 0) {
                best_node_split[0].clear();
                best_ftr_idx[0] = -1;
            }

            item.barrier(sycl::access::fence_space::local_space);

            for (Index ftr_idx = 0; ftr_idx < ftr_count; ++ftr_idx) {
                const Index ftr_position = node_id * ftr_count + ftr_idx;
                const Index ts_ftr_id = selected_ftr_list_ptr[ftr_position];

                split_scalar_t ftr_best;
                if (local_id == 0) ftr_best.clear();
                ...
                for (Index bin_ofs = 0; bin_ofs < bin_count; bin_ofs += bin_block) {
                    ...
                    clean_local_hists(
                        ...
                        local_weight.template get_multi_ptr<sycl::access::decorated::yes>().get_raw(),
                        local_id, wg_size, hist_prop_count, act_bin_block);

                    item.barrier(sycl::access::fence_space::local_space);

                    compute_histogram(local_hist,
                                      local_weight,
                                      item,
                                      ctx,
                                      act_bin_block,
                    ...
                    if (local_id < act_bin_block) {
                        hist_type_t* cur_hist =
                            local_hist.template get_multi_ptr<sycl::access::decorated::yes>().get_raw() +
                            local_id * hist_prop_count;

                        split_info_t ts;
                        ts.scalars.clear();
                        ts.scalars.ftr_id = ts_ftr_id;
                        ts.scalars.ftr_bin = local_id + bin_ofs;
                        if (is_weighted) ts.scalars.left_weight_sum = local_weight[local_id];
                        ts.init(cur_hist, hist_prop_count);

                        if constexpr (std::is_same_v<Task, task::classification>) {
                            Index left_count = 0;
                            for (Index k = 0; k < hist_prop_count; ++k) left_count += cur_hist[k];
                            ts.scalars.left_count = left_count;
                            sp_hlp.calc_imp_dec(ts, node_ptr, node_imp_list_ptr,
                                                class_hist_list_ptr, hist_prop_count,
                                                node_id, is_weighted);
                        } else {
                            ts.scalars.left_count = Index(cur_hist[0]);
                            if (cur_hist[0] > 0) cur_hist[1] /= cur_hist[0];
                            sp_hlp.calc_imp_dec(ts, node_ptr, node_imp_list_ptr,
                                                node_id, is_weighted);
                        }

                        local_bin_scalars[local_id] = ts.scalars;
                    }

                    for (Index step = act_bin_block / 2; step > 0; step >>= 1) {
                        item.barrier(sycl::access::fence_space::local_space);
                        if (local_id < step && (local_id + step) < act_bin_block) {
                            auto& s1 = local_bin_scalars[local_id];
                            auto& s2 = local_bin_scalars[local_id + step];
                    ...
                    if (local_id == 0) {
                        auto& block_best = local_bin_scalars[0];
                        if (sp_hlp.test_split_is_best(ftr_best, block_best, min_obs_leaf)) {
                            ftr_best = block_best;

                            const Index best_bin_local = ftr_best.ftr_bin - bin_ofs;
                            hist_type_t* src =
                                local_hist.template get_multi_ptr<sycl::access::decorated::yes>().get_raw() +
                                best_bin_local * hist_prop_count;
                            hist_type_t* dst = hists_ptr + ftr_position * hist_prop_count;

                            for (Index k = 0; k < hist_prop_count; ++k) {
                                dst[k] = src[k];
                            }
                        }
                    }
                ...
                if (local_id == 0) {
                    if (sp_hlp.test_split_is_best(best_node_split[0], ftr_best, min_obs_leaf)) {
                        best_node_split[0] = ftr_best;
                        best_ftr_idx[0] = ftr_idx;
                    }
                }
                item.barrier(sycl::access::fence_space::local_space);
            } 
            ...
            if (local_id == 0) {
                split_info_t bs;
                bs.scalars = best_node_split[0];

                const Index best_ftr_pos = node_id * ftr_count + best_ftr_idx[0];
                hist_type_t* const best_hist = hists_ptr + best_ftr_pos * hist_prop_count;

                bs.init(best_hist, hist_prop_count);
