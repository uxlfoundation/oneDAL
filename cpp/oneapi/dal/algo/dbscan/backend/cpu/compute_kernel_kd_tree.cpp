...
using daal_dbscan_kd_tree_t =
    daal_dbscan::internal::DBSCANBatchKernel<Float, daal_dbscan::kdTree, Cpu>;

template <typename Float, daal::internal::CpuType Cpu>
class dbscan_kd_tree_compute_wrapper {
public:
    ...
    auto compute(Args&&... args) {
        ...
        if (par->memorySavingMode == false) {
            return daal_dbscan::internal::DBSCANBatchKernel<Float, daal_dbscan::kdTree, Cpu>{}
...
static result_t call_daal_kernel(const context_cpu& ctx,
    ...
    interop::status_to_exception(interop::call_daal_kernel<Float, dbscan_kd_tree_compute_wrapper>(
        ctx,
        daal_data.get(),
        daal_weights.get(),
        daal_responses.get(),
