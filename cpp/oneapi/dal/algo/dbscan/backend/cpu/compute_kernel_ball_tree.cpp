...
using daal_dbscan_ball_tree_t =
    daal_dbscan::internal::DBSCANBatchKernel<Float, daal_dbscan::ballTree, Cpu>;

template <typename Float, daal::internal::CpuType Cpu>
class dbscan_ball_tree_compute_wrapper {
public:
    ...
    auto compute(Args&&... args) {
        ...
        if (par->memorySavingMode == false) {
            return daal_dbscan::internal::DBSCANBatchKernel<Float, daal_dbscan::ballTree, Cpu>{}
