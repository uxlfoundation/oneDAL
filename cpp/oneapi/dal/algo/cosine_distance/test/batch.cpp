...
#include "oneapi/dal/algo/cosine_distance/compute.hpp"

#include "oneapi/dal/test/engine/fixtures.hpp"
#include "oneapi/dal/test/engine/math.hpp"

namespace oneapi::dal::cosine_distance::test {

namespace te = dal::test::engine;
namespace la = te::linalg;

template <typename TestType>
class cosine_distance_batch_test
...
public:
    ...
    auto get_descriptor() const {
        return cosine_distance::descriptor<Float, Method>{};
    }
    ...
    void general_checks(const te::dataframe& x_data,
        ...
        const auto cosine_distance_desc = get_descriptor();

        INFO("run compute");
        const auto compute_result = this->compute(cosine_distance_desc, x, y);
        check_compute_result(x, y, compute_result);
    }

    void check_compute_result(const table& x_data,
                              const table& y_data,
                              const cosine_distance::compute_result<>& result) {
        ...
        for (std::int64_t i = 0; i < row_count_x; ++i) {
            for (std::int64_t j = 0; j < row_count_y; ++j) {
                ...
                for (std::int64_t k = 0; k < column_count; ++k) {
                    x_variance += x_data_matrix.get(i, k) * x_data_matrix.get(i, k);
                    y_variance += y_data_matrix.get(j, k) * y_data_matrix.get(j, k);
                    numerator += x_data_matrix.get(i, k) * y_data_matrix.get(j, k);
                }
...
TEMPLATE_LIST_TEST_M(cosine_distance_batch_test,
                     ...
                     "[cosine_distance][integration][batch]",
                     cosine_distance_types) {
...
TEMPLATE_LIST_TEST_M(cosine_distance_batch_test,
                     "cosine_distance compute one element matrix",
                     "[cosine_distance][integration][batch]",
                     cosine_distance_types) {
    SKIP_IF(this->not_float64_friendly());
...
TEMPLATE_LIST_TEST_M(cosine_distance_batch_test,
                     "cosine_distance self equals x-vs-x and empty-y",
                     "[cosine_distance][integration][batch]",
                     cosine_distance_types) {
    SKIP_IF(this->not_float64_friendly());
