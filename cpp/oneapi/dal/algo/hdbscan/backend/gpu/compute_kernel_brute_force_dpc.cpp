...
static result_t compute_kernel_dense_impl(const context_gpu& ctx,
    ...
    try {
        ...
        std::cout << "[hdbscan-gpu][brute_force] step1 BEGIN alloc dist_matrix (" << row_count
                  << " x " << row_count << ")" << std::endl;
        ...
        std::cout << "[hdbscan-gpu][brute_force] step2 END   compute_distance_matrix ok"
                  << std::endl;
        ...
        std::cout << "[hdbscan-gpu][brute_force] step4 END   compute_core_distances ok"
                  << std::endl;
        ...
        auto mrd_compute_event = compute_mrd_matrix<Float>(queue,
                                                           core_distances,
                                                           mrd_matrix,
                                                           metric,
                                                           alpha,
        ...
        std::cout << "[hdbscan-gpu][brute_force] step6 BEGIN alloc mst_from/to/weights ("
        ...
        auto mst_event = build_mst<Float>(
            queue,
            mrd_matrix,
            mst_from,
            mst_to,
        ...
        std::cout << "[hdbscan-gpu][brute_force] free dist_matrix BEGIN" << std::endl;
        dist_matrix = pr::ndarray<Float, 2>{};
        queue.wait_and_throw();
        std::cout << "[hdbscan-gpu][brute_force] free dist_matrix END" << std::endl;
        ...
        std::cout << "[hdbscan-gpu][brute_force] SYCL exception: " << e.what() << std::endl;
        throw;
    }
    catch (const std::exception& e) {
        std::cout << "[hdbscan-gpu][brute_force] std::exception: " << e.what() << std::endl;
        throw;
    }
}
