...
static result_t compute_kernel_ball_tree_impl(const context_gpu& ctx,
    ...
    try {
        ...
        for (std::int64_t b_start = 0; b_start < row_count; b_start += block_size) {
            ...
            std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx
                      << " BEGIN alloc ksel_vals (" << b_rows << " x " << k << ")" << std::endl;
        ...
        std::cout << "[hdbscan-gpu][ball_tree] SYCL exception: " << e.what() << std::endl;
        throw;
    }
    catch (const std::exception& e) {
        std::cout << "[hdbscan-gpu][ball_tree] std::exception: " << e.what() << std::endl;
        throw;
    }
}
