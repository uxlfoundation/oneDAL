...
static result_t compute_kernel_ball_tree_impl(const context_gpu& ctx,
    ...
    std::cout << "[hdbscan-gpu][ball_tree] enter compute_kernel_ball_tree_impl" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] descriptor read: row_count=" << row_count
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step0 BEGIN table2ndarray" << std::endl;
    const auto data_nd = pr::table2ndarray<Float>(queue, local_data, sycl::usm::alloc::device);
    queue.wait_and_throw();
    std::cout << "[hdbscan-gpu][ball_tree] step0 END   table2ndarray ok" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step1 block_size=" << block_size << " k=" << k
              << " needs_sqrt=" << needs_sqrt << std::endl;

    std::cout << "[hdbscan-gpu][ball_tree] step1 BEGIN alloc core_distances" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step1 END   alloc core_distances ok" << std::endl;
    ...
    for (std::int64_t b_start = 0; b_start < row_count; b_start += block_size) {
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " b_start=" << b_start
                  << " b_end=" << b_end << " b_rows=" << b_rows << std::endl;

        auto block_view = pr::ndview<Float, 2>::wrap(data_nd.get_data() + b_start * col_count,
                                                     { b_rows, col_count });

        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " BEGIN alloc dist_block ("
                  << b_rows << " x " << row_count << ")" << std::endl;
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " END   alloc dist_block ok"
                  << std::endl;

        sycl::event dist_event;
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " BEGIN distance op"
                  << std::endl;
        switch (metric) {
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " END   distance op ok"
                  << std::endl;

        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " BEGIN alloc ksel_vals ("
                  << b_rows << " x " << k << ")" << std::endl;
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " END   alloc ksel_vals ok"
                  << std::endl;

        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " BEGIN kselect_by_rows"
                  << std::endl;
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx << " END   kselect_by_rows ok"
                  << std::endl;
        ...
        std::cout << "[hdbscan-gpu][ball_tree] block #" << block_idx
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step1 END   blocked core dist loop ok (" << block_idx
              << " blocks)" << std::endl;

    // Step 2: GPU Boruvka MST with on-the-fly distance computation
    std::cout << "[hdbscan-gpu][ball_tree] step2 BEGIN alloc mst arrays" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step2 END   alloc mst arrays ok" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step3 BEGIN build_mst_otf" << std::endl;
    auto mst_event =
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step3 END   build_mst_otf ok" << std::endl;

    // Step 3: Sort MST edges by weight
    std::cout << "[hdbscan-gpu][ball_tree] step4 BEGIN sort_mst_by_weight" << std::endl;
    auto sort_event =
        sort_mst_by_weight<Float>(queue, mst_from, mst_to, mst_weights, edge_count, { mst_event });
    sort_event.wait_and_throw();
    std::cout << "[hdbscan-gpu][ball_tree] step4 END   sort_mst_by_weight ok" << std::endl;

    // Step 4: Extract flat clusters
    std::cout << "[hdbscan-gpu][ball_tree] step5 BEGIN alloc arr_responses" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step5 END   alloc arr_responses ok" << std::endl;

    std::cout << "[hdbscan-gpu][ball_tree] step6 BEGIN extract_clusters" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step6 END   extract_clusters ok" << std::endl;

    // Count clusters via GPU reduction
    std::cout << "[hdbscan-gpu][ball_tree] step7 BEGIN max-label reduction" << std::endl;
    ...
    std::cout << "[hdbscan-gpu][ball_tree] step7 END   max_label=" << max_label
              << " cluster_count=" << cluster_count << std::endl;

    std::cout << "[hdbscan-gpu][ball_tree] step8 BEGIN make_results" << std::endl;
    auto out = make_results<Float>(queue, desc, arr_responses, cluster_count, local_data);
    std::cout << "[hdbscan-gpu][ball_tree] step8 END   make_results ok" << std::endl;
    return out;
}
