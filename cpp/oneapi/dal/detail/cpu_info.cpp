...
cpu_info::cpu_info()
        :
#if defined(TARGET_X86_64)
          impl_(std::make_unique<cpu_info_x86>())
#elif defined(TARGET_ARM)
          impl_(std::make_unique<cpu_info_arm>())
#elif defined(TARGET_RISCV64)
          impl_(std::make_unique<cpu_info_riscv64>())
#endif
{
}
...
cpu_info::cpu_info(const cpu_extension cpu_extension_)
        :
#if defined(TARGET_X86_64)
          impl_(std::make_unique<cpu_info_x86>(cpu_extension_))
#elif defined(TARGET_ARM)
          impl_(std::make_unique<cpu_info_arm>(cpu_extension_))
#elif defined(TARGET_RISCV64)
          impl_(std::make_unique<cpu_info_riscv64>(cpu_extension_))
#endif
{
}
