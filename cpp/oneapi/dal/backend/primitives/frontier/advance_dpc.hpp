...
struct Context {
  ...
  inline size_t get_assigned_element(const ContextState& state) const {
    ...
    const int* bitmap_offsets = in_dev_frontier.get_offsets();
  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    while (context.need_to_process(state)) {
      ...
      if (context.check(state, assigned_vertex)) {
        uint32_t n_edges = graph_dev.getDegree(assigned_vertex); // GRAPH degree of the vertex
      ...
      for (size_t i = 0; i < wg_tail.load(); i++) {
        ...
        for (auto j = lid; j < n_edges; j += wgroup_size) {
          auto n = start + j;
          auto edge = n.getIndex(); // GRAPH edge index
          auto weight = graph_dev.getEdgeWeight(edge); // GRAPH edge weight
          auto neighbor = *n;
...
sygraph::Event launchBitmapKernel(GraphT& graph, const InFrontierT& in, const OutFrontierT& out, LambdaT&& functor, size_t expected_size) {
  ...
  auto graph_dev = graph.getDeviceGraph(); // GRAPH device view

  size_t coarsening_factor = types::detail::COMPUTE_UNIT_SIZE / 16; // sygraph::detail::device::getSubgroupSize(q);

  sycl::event to_wait;
  sycl::range<1> local_range;
  size_t global_size;
  size_t bitmap_range = in.get_element_bitsize();
