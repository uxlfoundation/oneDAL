...
template<typename VertexT = std::uint32_t, typename EdgeT = std::uint32_t, typename WeightT = std::uint32_t>
...
public:
  csr_graph_view(std::size_t num_nodes,
                 vertex_t* row_ptr,
                 edge_t* col_indices,
                 weight_t* weights)
...
public:
  csr_graph(sycl::queue& queue,
            ...
            : _queue(queue), _num_nodes(row_ptr.size() - 1) {
