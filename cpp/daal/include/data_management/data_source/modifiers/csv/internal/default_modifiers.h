...
{
public:
    DAAL_DATA_TYPE apply(Context & /*context*/, size_t /*index*/) override { return (DAAL_DATA_TYPE)0; }
};
...
{
public:
    void initialize(Config & config, size_t index) override { config.setOutputFeatureType(index, features::DAAL_CONTINUOUS); }

    DAAL_DATA_TYPE apply(Context & context, size_t index) override { return context.getTokenAs<DAAL_DATA_TYPE>(index); }
};
...
public:
    ...
    void initialize(Config & config, size_t index) override { config.setOutputFeatureType(index, features::DAAL_CATEGORICAL); }

    DAAL_DATA_TYPE apply(Context & context, size_t index) override
    {
    ...
    void finalize(Config & config, size_t index) override
    {
