...
services::Status corDistanceUpperPacked(const NumericTable * xTable, const NumericTable * yTable, NumericTable * rTable)
{
    ...
    daal::threader_for(nBlocks1, nBlocks1, [=, &safeStat](size_t k1) {
        ...
        for (size_t k2 = 0; k2 < nBlocks2; k2++)
        {
            ...
            /* Copy results to upper-packed format */
            ...
            for (size_t i = 0; i < blockSize1; i++)
            {
                for (size_t j = 0; j < blockSize2; j++)
                {
                    ...
                    if (row <= col) // Only store in upper triangular part
                    {
                        r[(col * (col + 1)) / 2 + row] = rbuf[i * blockSize2 + j];
                    }
                }
            }
