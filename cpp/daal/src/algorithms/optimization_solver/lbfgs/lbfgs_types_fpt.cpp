...
DAAL_EXPORT services::Status Result::allocate(const daal::algorithms::Input * input, const daal::algorithms::Parameter * par, const int method)
{
    ...
    if (!pTbl.get())
    {
        pTbl = data_management::HomogenNumericTable<int>::create(2, 1, data_management::NumericTable::doAllocate, 0);
        pOpt->set(correctionIndices, pTbl);
    }
    pTbl = data_management::NumericTable::cast(pOpt->get(averageArgumentLIterations));
    if (!pTbl.get())
    {
        pTbl = data_management::HomogenNumericTable<algorithmFPType>::create(argumentSize, 2, data_management::NumericTable::doAllocate, 0.0);
