...
# Both .pc files are linked and run. The static one is the reason the oneMKL
# archives are named with `-l:` inside a link group: with plain `-lmkl_core
# -lmkl_intel_ilp64 -lmkl_tbb_thread`, `-l` finds oneMKL's *shared* libraries,
# whose internals live in the kernel libraries oneMKL dlopen's at runtime
# (`libmkl_def.so`, `libmkl_avx*.so`), and the link fails with ~9600
#
#   /usr/bin/ld: .../libmkl_tbb_thread.so: undefined reference to
#       `mkl_spblas_dcsr0ng__c__mmout_par'
#
# Naming the archives without a group leaves ~400 of those and repeating them
# once leaves ~30, so the group is doing real work rather than papering over an
# ordering mistake. This test is the only coverage of that link: nothing else in
# CI, in either build system, links the static .pc.
for pc in dal-static-threading-host dal-dynamic-threading-host; do
