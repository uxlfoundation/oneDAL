...
#endif
  return ((xcr0 & mask) ==
          mask); /* checking if xmm and ymm state are enabled in XCR0 */
}
...
int check_avx512_features() {
  ...
  */
  ...
  /*
  E0H - KMASK state, upper 256-bit of ZMM0-ZMM15 and ZMM16-ZMM31 state are
  enabled by OS 06H - XMM state and YMM state are enabled by OS
  */
