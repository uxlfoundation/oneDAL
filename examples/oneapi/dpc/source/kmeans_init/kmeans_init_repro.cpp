...
void run(sycl::queue& q, const std::vector<float>& data,
         std::int64_t n_samples, std::int64_t n_features, std::int64_t n_clusters) {
    constexpr std::int64_t max_iter = 5;

    auto x_table = dal::homogen_table::wrap(data.data(), n_samples, n_features);
    std::cout << "Table: rows=" << x_table.get_row_count()
              << " cols=" << x_table.get_column_count() << std::endl;

    // Step 1: kmeans++ init
    const auto init_desc = dal::kmeans_init::descriptor<float,
    ...
    std::cout << "Init done! Centroids shape: "
              << init_result.get_centroids().get_row_count() << " x "
              << init_result.get_centroids().get_column_count() << std::endl;
    ...
    const auto train_result = dal::train(q, kmeans_desc, x_table,
                                          init_result.get_centroids());

    std::cout << "Train done! Iterations: " << train_result.get_iteration_count()
              << ", Objective: " << train_result.get_objective_function_value()
              << std::endl;
}
...
int main(int argc, char const* argv[]) {
    ...
    if (fin.good()) {
        ...
        std::cout << "Loaded test data from " << data_file
    ...
    std::cout << "Data: " << n_samples << " x " << n_features
              << ", clusters=" << n_clusters << ", float32" << std::endl;
