...
#define ONEDAL_LIBS                                                            \
  PATH(onedal) PATH(onedal_core) PATH(onedal_thread) PATH(onedal_parameters)
#else
...
#define LIBS_LINE                                                              \
  -Wl, --start - group ONEDAL_LIBS MATH_LIBS - Wl, --end - group OTHER_LIBS
#else
#define SUFFIX so
#define OTHER_LIBS                                                             \
  -lmkl_core - lmkl_intel_lp64 - lmkl_tbb_thread - ltbb - ltbbmalloc -         \
      lpthread - ldl
...
#define OTHER_LIBS                                                             \
  -lmkl_core - lmkl_intel_lp64 - lmkl_tbb_thread - ltbb - ltbbmalloc - ldl
#ifdef PARAMETERS_LIB
#define ONEDAL_LIBS                                                            \
  PATH(onedal) PATH(onedal_core) PATH(onedal_thread) PATH(onedal_parameters)
#else
...
#define OPTS                                                                   \
  -std = c++ 17 - Wno - deprecated - declarations - diag - disable = 10441

#elif defined(_WIN32) || defined(_WIN64)
#define LIBDIR lib / intel64

#define OTHER_LIBS                                                             \
  mkl_core.lib mkl_intel_lp64.lib mkl_tbb_thread.lib tbb12.lib tbbmalloc.lib
...
prefix = ${pcfiledir} /../../ exec_prefix = ${prefix} libdir =
             ${exec_prefix} / LIBDIR includedir = ${prefix} / include

                                                                  Name
    : oneDAL Description : oneAPI Data Analytics Library Version : 2026.2 URL
    : ONEDAL_URL Libs : LIBS_LINE Cflags : OPTS - I$ {
  includedir
}
